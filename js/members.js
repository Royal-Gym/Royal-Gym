...
// ------------------------------------------------------------
const getMembers = () => {
  // Step 1: Use localStorage.getItem("members") to get the raw JSON string

  // Step 2: Parse the JSON string with JSON.parse()

  // Step 3: Return the parsed array
  //         → If the key doesn't exist, return an empty array []
  //         → Example: return JSON.parse(localStorage.getItem("members")) || [];
};
...
const saveMembers = (data) => {
  // Step 1: Stringify the data array using JSON.stringify(data)

  // Step 2: Save to localStorage using localStorage.setItem("members", ...)
};
